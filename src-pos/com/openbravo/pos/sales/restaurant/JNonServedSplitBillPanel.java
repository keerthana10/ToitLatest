 ...
 * @author shilpa
 */
public class JNonServedSplitBillPanel extends JDialog implements ListSelectionListener {
...
//    } 
        ...
        public static String showMessage(Component parent,String tableId,List<SharedSplitTicketInfo> tinfo){
        parentLocal = parent;
        tableID=tableId;
        tinfoLocal=tinfo;
      Window window = getWindow(parent);
        JNonServedSplitBillPanel myMsg;
        if (window instanceof Frame) {
            myMsg = new JNonServedSplitBillPanel((Frame) window, true);
        } else {
            myMsg = new JNonServedSplitBillPanel((Dialog) window, true);
        }
        String splitId=myMsg.initNonServedLines(tableId,tinfo);
        return splitId;
...
//    }
   ...
   private JNonServedSplitBillPanel(Frame frame, boolean b) {
       super(frame, true);
       setBounds(x, y, width, height);
    }
    private JNonServedSplitBillPanel(Dialog dialog, boolean b) {
        super(dialog, true);
        setBounds(x, y, width, height);
...
//    }
      ...
      public String initNonServedLines(String tableId,List<SharedSplitTicketInfo> tinfo) {
        initComponents();
        setTitle("Split Bill Selector");
        jSplitTable1.getTableHeader().setBackground(Color.white);
        setDataTableModelAndHeader(jSplitTable1,tinfo.size());
        setNonServedSplitPrimaryData(jSplitTable1, tableId,tinfo);
        setVisible(true);
        return splitId;
    }
    ...
    private void setDataTableModelAndHeader(JTable table, int size) {
      ...
      private void setNonServedSplitPrimaryData(JTable jTable3,String placeName, List<SharedSplitTicketInfo> sharedticket) {
      // System.out.println("testing window 4"+tinfoLocal.size()+tinfoLocal.get(0).getContent().getName());
       int ticketlinesSize=sharedticket.size();  
           for (int col = 0; col <  ticketlinesSize; col++) {
            jTable3.setRowHeight(col, 30);   
