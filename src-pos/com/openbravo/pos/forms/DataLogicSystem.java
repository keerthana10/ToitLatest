...
public class DataLogicSystem extends BeanFactoryDataSingle {
    ...
    protected SentenceExec m_insertday;
    protected SentenceFind m_sequenceday;
    protected SentenceFind m_activeday;
    ...
    public void init(Session s){
        ...
        m_insertcash = new StaticSentence(s
                , "INSERT INTO CLOSEDCASH(MONEY, HOST, HOSTSEQUENCE, POSNO, DATESTART, DATEEND) " +
                  "VALUES (?, ?, ?, ?, ?, ?)"
                , new SerializerWriteBasic(new Datas[] {Datas.STRING, Datas.STRING, Datas.INT, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP}));
          m_sequenceday = new StaticSentence(s,
                "SELECT MAX(HOSTSEQUENCE) FROM CLOSEDDAY WHERE HOST = ?",
                SerializerWriteString.INSTANCE,
                SerializerReadInteger.INSTANCE);
        m_activeday = new StaticSentence(s
            , "SELECT HOST, HOSTSEQUENCE, DATESTART, DATEEND FROM CLOSEDDAY WHERE MONEY = ?"
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.INT, Datas.TIMESTAMP, Datas.TIMESTAMP}));
        m_insertday = new StaticSentence(s
                , "INSERT INTO CLOSEDDAY(MONEY, HOST, HOSTSEQUENCE, POSNO, DATESTART, DATEEND) " +
                  "VALUES (?, ?, ?, ?, ?, ?)"
                , new SerializerWriteBasic(new Datas[] {Datas.STRING, Datas.STRING, Datas.INT, Datas.STRING, Datas.TIMESTAMP, Datas.TIMESTAMP}));

        Datas[] loginDatas = new Datas[] {Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING, Datas.STRING };
       m_peoplelogInsert = new PreparedSentence(s
                , "INSERT INTO PEOPLELOG (ID, PEOPLE_ID, STARTTIME, REASON, POSNO) VALUES (?, ?, ?, ?, ?)"
...
//    public abstract BaseSentence getShutdown();
    ...
    public final void peoplelogInsert(String Id,String peopleId, String startTime,String reason, String posNo) throws BasicException {

       Object[] value = new Object[] {Id, peopleId, startTime, reason, posNo};
    ...
    public final int getSequenceDay(String host) throws BasicException {
        Integer i = (Integer) m_sequenceday.find(host);
        return (i == null) ? 1 : i.intValue();
    }
     public final Object[] findActiveDay(String sActiveCashIndex) throws BasicException {
        return (Object[]) m_activeday.find(sActiveCashIndex);
    }
     public final void execInsertDay(Object[] cash) throws BasicException {
        m_insertday.exec(cash);
    }
